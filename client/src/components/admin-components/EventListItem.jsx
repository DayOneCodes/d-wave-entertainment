...
export default function EventListItem ({ event, status, deleteEvent, updateEvent, showModal, changeModal, closeModal }) {
  ...
  return (
    ...
    <span className="size-1.5 rounded-full bg-white animate-pulse"></span> {status}
                                    </span>
    </div>
    </div>
    ...
    <button className="w-full md:w-auto flex items-center justify-center gap-2 px-4 py-2 border-2 border-primary text-primary text-sm font-bold rounded-lg" onClick={() => updateEvent()}>
    <span className="material-symbols-outlined text-[20px]">edit</span>
                                    Edit
                                </button>
    <button className="w-full md:w-auto flex items-center justify-center gap-2 px-4 py-2 bg-red-700 text-white border-2 border-primary text-sm font-bold rounded-lg hover:bg-red-600 transition-all active:scale-95" 
    onClick={() => {
      showModal(<ConfirmDelete event={event} deleteEvent={deleteEvent} changeModal={changeModal} closeModal={closeModal}/>)
    }}>
    <span 
